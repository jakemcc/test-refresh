...
Instead of commenting out the others or quitting lein-test-refresh and restarting with a test-selector you can add `:test-refresh/focused true` to your test(s) or namespace.
...
A [sample.project.clj](sample.project.clj) contains the definitive example of configuring `lein-test-refresh` features.
...
Often `lein-test-refresh` configuration is a personal preference and should be configured in your personal `~/.lein/profiles.clj`.

### Notifications

`lein-test-refresh` supports specifying a notification command.
...
`lein-test-refresh` also has built-in Growl support.
...
`lein-test-refresh` can be configured to suppress `clojure.test`'s _Testing namespace_ output.
...
`lein-test-refresh` can be configured to use a custom `clojure.test` output report. 
...
At first this seems like a weird feature for a refreshing test runner to support but because of the other features `lein-test-refresh` supports, such as custom test runners, being able to just run tests once can be useful.
See [this](https://github.com/jakemcc/lein-test-refresh/pull/48) pull request for discussion.
...
`lein-test-refresh` supports running in a repl.
This was done to support running in Cursive's repl so [users could click](https://github.com/jakemcc/lein-test-refresh/issues/80) and navigate to source.

To use this feature, add `lein-test-refresh` as a project dependency instead of as a plugin.
...
Working on `lein-test-refresh` can be a bit tricky.
...
1. In `./test-refresh` run `lein install` to put a version built from your local `lein-test-refresh` checkout into your `~/.m2` directory.
...
If there were more active changes happening to the project I'd invest the time to figure out how to test it but given the stability of `lein-test-refresh` I haven't bothered.
...
Because of [tools.namespace](https://github.com/clojure/tools.namespace) changes `lein-test-refresh` requires that your project use Clojure >= 1.3.0.
